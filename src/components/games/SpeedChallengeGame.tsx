...
export function SpeedChallengeGame({ onComplete, onBack, words: allWords }: SpeedChallengeGameProps) {
  ...
  const shuffledWords = useMemo(() => {
    return [...allWords, ...allWords].sort(() => Math.random() - 0.5);
  }, [allWords]);
  ...
  const endGame = () => { setIsGameOver(true); onComplete(score); };

  const restartGame = () => {
    setCurrentIndex(0); setScore(0); setStreak(0); setMaxStreak(0);
    setTimeLeft(60); setIsGameOver(false); setShowFeedback(null);
  };
      ...
      <div className="max-w-2xl mx-auto text-center py-12">
        <div className="bg-card rounded-3xl shadow-card p-8">
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            ...
            <div className="bg-secondary rounded-xl p-4">
              <div className="text-2xl font-bold text-green-500">{currentIndex}</div>
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className={cn(
        'bg-card rounded-3xl shadow-card p-8 transition-all duration-200 relative',
        showFeedback === 'correct' && 'ring-4 ring-green-500',
        ...
        <div className="grid grid-cols-2 gap-4">
          {options.map((option, index) => (
            <button key={index} onClick={() => handleAnswer(option)} disabled={showFeedback !== null}
              className="p-5 rounded-xl border-2 text-left transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] bg-secondary border-transparent hover:bg-secondary/80">
              <span className="text-lg">{option}</span>
            </button>
          ))}
        </div>
      </div>
      <p className="text-center text-muted-foreground mt-6">快速选择正确的释义，连击可获得额外分数！</p>
    </div>
  );
}
